...
public class ReverseWordsInString {
    ...
    // returns a string that has these words in reverse order. For example, given the string "tim
    // is great", your function should return "great is tim".
    // For this problem, a word can contain special characters, punctuation, and numbers. The
    // words in the string will be separated by one or more whitespaces, and the reversed string
    ...
    public String reverseWordsInString(String string) {
        ...
        for (char ch : chars) {
            if (Character.isWhitespace(ch)) {
                if (!buf.isEmpty()) {
                    accumulator.insert(0, buf);
                    buf.setLength(0);
                }
                accumulator.insert(0, ch);
            } else {
                buf.append(ch);
            }
        }
